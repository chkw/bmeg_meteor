...
Template.obsDeckTemplate.onRendered(function() {
    ...
    if (use_case == 2) {
        ...
        innerHTML = innerHTML +
            "The displayed sample data includes expression and mutation data in the specified genes as well as any drug sensitivity signatures where the scores differ by KS test for sample group with variant compared to sample group without variant.";
    } else {
        innerHTML = innerHTML + "<br>";
        innerHTML = innerHTML + "selected signatures: " + selectedSigs;
        innerHTML = innerHTML + "<br>";
        innerHTML = innerHTML +
            "The displayed sample data includes the expression level of the query gene set as well as some top-weighted genes from the signature.";
    }

    document.getElementById("queryP")
        .innerHTML = innerHTML;

    // get data via the Meteor.method
    if (use_case == 2) {
        console.log("use_case 2");
        Meteor.call("query_get_event_data", selectedSigs, sessionGeneList, [
            "submittedTumorSite"
        ], buildObsDeckWithData);
    } else {
